...
from trl import BasePairwiseJudge, OnlineDPOConfig
from trl.trainer.utils import disable_dropout_in_model, DPODataCollatorWithPadding

# 这一部分省略了很多，尤其是is_sagemaker_mp_enabled，apex
...
class OnlineDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if ref_model is model:
            raise ValueError("model不能和ref_model一样")

        if reward_model is None and judge is None:
            raise ValueError("reward_model和judge必须提供一个")
        ...
        if args is None or processing_class is None:
            raise ValueError("`args` `processing_class` must be provided.")

        # peft_config默认不使用，使用的时候再加
        # 不考虑没提供ref_model的问题
        self.ref_model = ref_model
        self.ref_model.eval()
        self.reward_model = reward_model
        self.reward_processing_class = reward_processing_class
        self.judge = judge   # judge和reward_model只能留一个，否则后面会炸

        if peft_config is not None:
            if not is_peft_available():
                raise ImportError("PEFT不可用，即is_peft_available()=False，此时别提供peft_config")
    ...
    # @staticmethod用于定义静态方法，该方法可以在类没实例话的时候调用该函数
    @staticmethod
    def tokenize_row(feature, is_encoder_decoder: bool, tokenizer: PreTrainedTokenizerBase) -> Dict[str, Any]:
        """Tokenize a single row from a DPO specific dataset."""
        if not is_encoder_decoder:
